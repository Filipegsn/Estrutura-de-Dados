...
int dd, numero;
 ...
 struct aniversario{
int dia, ano; char mes[10];
};
 struct dados{
char nome[20], email[20], OBS[50];
struct telefone tel;
struct endereco ed;
...
{
    struct dados d[100];
    int op, op2, indice=0, i, x, encontrou=0, menor, pesq0, cod, sair;
    char pesq[20], pesq2[30];
...
do{
    ...
    switch(op){
        ...
        case 5:
            ...
            printf("\n NOME: %s",d[cod].nome);
            printf("\n EMAIL: %s",d[cod].email);
            printf("\n TEL: %i %i",d[cod].tel.dd, d[i].tel.numero);
            printf("\n ANIVERSARIO: %d %s %d", d[i].dt.dia, d[i].dt.mes, d[i].dt.ano);
            printf("\n ENDERECO: %s %s %s %s %s", d[i].ed.rua, d[i].ed.bairro, d[i].ed.city, d[i].ed.state, d[i].ed.pais);
            printf("\nTEM CERTEZA ? SIM(1)/NAO(2)");
            scanf("%i", &i);
            if (i==1){
